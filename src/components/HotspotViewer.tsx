...
export const HotspotViewer: React.FC<HotspotViewerProps> = ({
  ...
  // Load custom stored image from IndexedDB if present
  useEffect(() => {
    const baseName = cleanKey.replace(/\.[a-zA-Z0-9]+$/, '');
    Promise.all([
      getMediaObjectURL(cleanKey),
      getMediaObjectURL(baseName),
      getMediaObjectURL('asset_015'),
      getMediaObjectURL('asset_015.jpeg'),
      getMediaObjectURL('petunjuk_image'),
      getMediaObjectURL('Gemini_Generated_Image_m2ivxjm2ivxjm2iv.jpg')
    ]).then(([u1, u2, u3, u4, u5, u6]) => {
      const found = u1 || u2 || u3 || u4 || u5 || u6;
      if (found) {
        setCustomUrl(found);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await saveMediaBlob('asset_015', file);
    await saveMediaBlob('asset_015.jpeg', file);
    await saveMediaBlob('petunjuk_image', file);
    await saveMediaBlob('Gemini_Generated_Image_m2ivxjm2ivxjm2iv.jpg', file);
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      >
          ...
          <div className="relative w-full aspect-[16/9] min-h-[400px] bg-gradient-to-b from-sky-300 via-sky-100 to-emerald-200 flex items-stretch overflow-hidden border border-emerald-400">
            ...
            <div className="relative z-10 w-full h-full flex flex-col sm:flex-row items-center justify-between p-4 sm:p-8 gap-4">
              ...
              <div className="flex-1 w-full max-w-xl bg-amber-100/95 border-4 border-amber-900 rounded-3xl p-4 sm:p-5 shadow-2xl flex flex-col gap-2 relative">
                ...
                {/* Bottom Wooden Sign "PETA DESA" & Upload Prompt */}
          ...
          >
            ...
            <div className="bg-gradient-to-r from-emerald-800 to-teal-900 px-3 py-2 flex items-center justify-between border-b border-emerald-700/60">
              ...
              <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                <HotspotPhotoUploader hotspot={hoveredHotspot} compact={true} />
        ...
        <button
          ...
          className="absolute z-20 rounded-xl cursor-pointer hover:bg-white/10 active:bg-white/20 transition-colors border-2 border-transparent hover:border-emerald-300/60"
