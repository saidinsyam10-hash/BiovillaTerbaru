...
export const TitleScreen: React.FC<TitleScreenProps> = ({ 
  ...
  // Load custom stored video or poster from IndexedDB if available
  useEffect(() => {
    getMediaObjectURL('asset_001').then(url => {
      if (url) {
        setVideoUrl(url);
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-lg border-2 border-emerald-200 bg-slate-900 aspect-video flex items-center justify-center group">
              {!videoError ? (
                ...
                <div className="p-6 text-center text-slate-300 flex flex-col items-center justify-center">
                  <div className="w-14 h-14 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 mb-3">
                    <BookOpen className="w-7 h-7" />
                  </div>
                  <h4 className="text-base font-bold text-white mb-1">Video Sambutan Pengelola Desa</h4>
                  <p className="text-xs text-slate-400 max-w-xs mb-3">
                    File video <code>asset 001.mp4</code> siap dimuat. Anda dapat meletakkannya di <code>/public/assets/</code> atau memilih langsung dari berkas perangkat.
                  </p>
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs shadow cursor-pointer transition-all active:scale-95"
                  >
                    <Upload className="w-3.5 h-3.5" />
                    <span>Pilih Berkas Video (asset 001)</span>
                  </button>
                </div>
              )}
            </div>
