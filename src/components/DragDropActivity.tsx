...
export const DragDropActivity: React.FC<DragDropActivityProps> = ({
  ...
  }, [task.backgroundImage, task.elements]);
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
      ...
      {task.backgroundImage && !bgError && (
        <div className="rounded-2xl overflow-hidden border-2 border-emerald-200 shadow-md bg-slate-900 max-h-48 flex items-center justify-center">
          <img
            src={resolvedBg}
            alt="Ilustrasi Misi"
                  ...
                  {zone.image && (
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex-shrink-0 flex items-center justify-center p-1 relative shadow-inner">
                      {!zoneImgErrors[zone.id] ? (
                        <img
                          src={resolveMediaPath(zone.image)}
                          alt={zone.label}
