...
const QUESTIONS: QuestionData[] = [
  // Babak 1 – Observasi Gejala (2 Soal)
  {
    id: 'q1',
    round: 1,
    roundTitle: 'Babak 1 – Observasi Gejala',
    prompt: 'Berdasarkan hasil pemindaian tiga gejala fisik di Desa Sel, manakah indikasi yang secara langsung mencerminkan penurunan pasokan energi ATP secara masif?',
    options: [
      { id: 'a', text: 'Penerangan fasilitas meredup, putaran mesin molekuler melambat, dan transpor aktif gerbang melemah', isCorrect: true },
      { id: 'b', text: 'Peningkatan sintesis materi genetik di balai desa dan pembesaran ukuran vakuola', isCorrect: false },
      { id: 'c', text: 'Pemisahan dinding sel dan percepatan pembelahan organel secara spontan', isCorrect: false },
      { id: 'd', text: 'Penumpukan cadangan pati dan pembentukan kloroplas baru di seluruh penjuru desa', isCorrect: false },
    ],
    explanation: 'Semua proses kerja seluler — mulai dari biosintesis (lampu menyala), pergerakan motor molekuler (roda gigi), hingga pompa ion pada membran (gerbang) — membutuhkan suplai energi kontinu dari hidrolisis ATP.'
  },
  {
    id: 'q2',
    round: 1,
    roundTitle: 'Babak 1 – Observasi Gejala',
    prompt: 'Mengapa krisis yang terjadi pada Pembangkit Energi (mitokondria) dapat melumpuhkan berbagai fasilitas lain di seluruh Desa Sel?',
    options: [
      { id: 'a', text: 'Karena ATP merupakan "mata uang energi" universal yang diperlukan hampir semua organel untuk transpor aktif, biosintesis, dan mobilitas', isCorrect: true },
      { id: 'b', text: 'Karena mitokondria menyimpan seluruh cetak biru genetik yang mengatur identitas organel lain', isCorrect: false },
      { id: 'c', text: 'Karena mitokondria berfungsi menyaring seluruh limbah beracun sebelum masuk ke vakuola', isCorrect: false },
      { id: 'd', text: 'Karena mitokondria menyintesis lipid penyusun membran pelindung balai desa', isCorrect: false },
    ],
    explanation: 'Mitokondria adalah produsen utama ATP. Tanpa ATP, reaksi-reaksi endergonik di ribosom, retikulum endoplasma, dan badan golgi tidak memiliki daya dorong termodinamika untuk berlangsung.'
  },
...
type SceneStep = 'scene1' | 'scene2' | 'scene3' | 'scene4' | 'scene5' | 'summary';
...
export const Level4EnergyCrisis: React.FC<Level4EnergyCrisisProps> = ({
  ...
  const [currentScene, setCurrentScene] = useState<SceneStep>('scene1');

  // Animation states for Scene 1 (Crisis Intro)
  const [scene1State, setScene1State] = useState<'normal' | 'flicker' | 'crisis'>('normal');

  // Interactive symptoms inspected in Scene 3 (Babak 1)
  const [inspectedSymptom, setInspectedSymptom] = useState<'bulb' | 'gear' | 'gate' | null>('bulb');
  ...
  const [viewModeBabak2, setViewModeBabak2] = useState<'photo' | 'schematic'>('photo');
    ...
    <div className="w-full flex flex-col space-y-4">
      ...
      <div className="flex items-center justify-between bg-slate-900/90 border border-teal-500/30 rounded-2xl p-2.5 sm:p-3 text-xs">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto py-0.5">
          {[
            { id: 'scene1', label: '1. Intro' },
            { id: 'scene2', label: '2. Transisi' },
            { id: 'scene3', label: '3. Gejala' },
            { id: 'scene4', label: '4. Mekanisme' },
            { id: 'scene5', label: '5. Prediksi' },
      ...
      {/* ========================================================================= */}
      {/* ADEGAN 1: INTRO KRISIS ENERGI                                              */}
      {/* ========================================================================= */}
      {currentScene === 'scene1' && (
        <div className="space-y-4">
          {/* Visual Asset Canvas (3D Isometrik Pixar-like, Pure Visual Without Text Overlay) */}
          <div className="relative w-full aspect-video max-h-[460px] bg-gradient-to-b from-[#042f2e] via-[#0f172a] to-[#1e1b4b] rounded-3xl overflow-hidden border-2 border-teal-500/40 shadow-2xl flex items-center justify-center select-none">
            {/* Ambient Bioluminescent Background Aura */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-teal-500/20 via-purple-900/10 to-transparent pointer-events-none" />
            ...
            <svg viewBox="0 0 960 540" className="w-full h-full object-cover">
              <defs>
                ...
                <linearGradient id="groundGrad" x1="0" y1="0" x2="1" y2="1">
                  ...
                  <stop offset="50%" stopColor="#115e59" stopOpacity="0.5" />
                  <stop offset="100%" stopColor="#042f2e" stopOpacity="0.8" />
                </linearGradient>

                <linearGradient id="mitoBodyGrad" x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stopColor={scene1State === 'crisis' ? '#ea580c' : '#f59e0b'} />
                  <stop offset="60%" stopColor={scene1State === 'crisis' ? '#b45309' : '#0d9488'} />
                  <stop offset="100%" stopColor="#4c1d95" />
                </linearGradient>

                <radialGradient id="beaconGlow" cx="50%" cy="50%" r="50%">
                  <stop offset="0%" stopColor={scene1State === 'crisis' ? '#ff4500' : '#10b981'} stopOpacity="0.9" />
                  <stop offset="100%" stopColor={scene1State === 'crisis' ? '#ff4500' : '#10b981'} stopOpacity="0" />
                </radialGradient>
              ...
              <g transform="translate(480, 290)">
                <path d="M 0 -170 L 380 0 L 0 170 L -380 0 Z" fill="url(#groundGrad)" stroke="#14b8a6" strokeWidth="2" strokeOpacity="0.4" />
                <path d="M 0 -110 L 250 0 L 0 110 L -250 0 Z" fill="none" stroke="#2dd4bf" strokeWidth="1" strokeDasharray="6 6" strokeOpacity="0.5" />
                
                {/* Pulsing Energy Conduits / Jalur Energi */}
                <path 
                  d="M -300 -10 Q -150 -50 0 0" 
                  fill="none" 
                  stroke={scene1State === 'crisis' ? '#f97316' : '#2dd4bf'} 
                  strokeWidth="5" 
                  strokeDasharray="8 6" 
                  filter="url(#softGlow)"
                  className={scene1State === 'crisis' ? 'opacity-40 animate-pulse' : 'opacity-90'} 
                />
                <path 
                  d="M 300 -10 Q 150 -50 0 0" 
                  fill="none" 
                ...
                {/* Facility 1: Production Plant (Left) */}
                <g transform="translate(-230, -50)">
                  <ellipse cx="0" cy="15" rx="35" ry="18" fill="#042f2e" opacity="0.6" />
                  <path d="M -25 0 L 0 -40 L 25 0 L 0 20 Z" fill={scene1State === 'crisis' ? '#334155' : '#0d9488'} stroke="#5eead4" strokeWidth="1.5" />
                  <circle cx="0" cy="-45" r="8" fill={scene1State === 'crisis' ? '#64748b' : '#38bdf8'} filter="url(#softGlow)" />
                </g>

                {/* Facility 2: Distribution Port (Right) */}
                <g transform="translate(230, -50)">
                  <ellipse cx="0" cy="15" rx="35" ry="18" fill="#042f2e" opacity="0.6" />
                  <path d="M -30 -10 L 0 -45 L 30 -10 L 0 25 Z" fill={scene1State === 'crisis' ? '#334155' : '#7e22ce'} stroke="#c084fc" strokeWidth="1.5" />
                  <circle cx="0" cy="-50" r="8" fill={scene1State === 'crisis' ? '#64748b' : '#fbbf24'} filter="url(#softGlow)" />
                </g>

                {/* Facility 3: Gate Outpost (Bottom) */}
                <g transform="translate(0, 110)">
                  <ellipse cx="0" cy="15" rx="35" ry="18" fill="#042f2e" opacity="0.6" />
                  <path d="M -25 -5 L 0 -35 L 25 -5 L 0 20 Z" fill={scene1State === 'crisis' ? '#334155' : '#14b8a6'} stroke="#2dd4bf" strokeWidth="1.5" />
                  <circle cx="0" cy="-40" r="7" fill={scene1State === 'crisis' ? '#64748b' : '#34d399'} filter="url(#softGlow)" />
                </g>

                {/* Center Power Plant (Pembangkit Energi / Mitokondria 3D Pixar Style) */}
                <g transform="translate(0, -35)">
                  {/* Outer Shadow */}
                  <ellipse cx="0" cy="45" rx="90" ry="40" fill="#020617" opacity="0.75" />

                  {/* Warning Beacons Ring */}
                  <circle cx="-65" cy="-20" r="14" fill="url(#beaconGlow)" />
                  <circle cx="65" cy="-20" r="14" fill="url(#beaconGlow)" />

                  {/* Main Mitochondrial Body with Cristae folds */}
                  <g className={scene1State === 'crisis' ? 'animate-pulse' : ''}>
                    <ellipse cx="0" cy="0" rx="80" ry="48" fill="url(#mitoBodyGrad)" stroke="#fde047" strokeWidth="3" filter="url(#softGlow)" />
                    {/* Inner cristae membrane convolutions */}
                    <path d="M -55 -10 Q -30 -30 -15 -5 Q 0 25 15 -10 Q 35 -30 55 -5" fill="none" stroke="#fef08a" strokeWidth="4" strokeLinecap="round" />
                    <path d="M -45 15 Q -20 -5 -5 18 Q 20 5 45 15" fill="none" stroke="#fef08a" strokeWidth="4" strokeLinecap="round" />
                  </g>
                  ...
                  <circle 
                    ...
                    cy="-60" 
                    r="12" 
                    fill={scene1State === 'crisis' ? '#ff3b30' : '#10b981'} 
                    stroke="#ffffff" 
                    strokeWidth="2" 
                    filter="url(#softGlow)"
                    className={scene1State === 'crisis' ? 'animate-ping' : ''}
                  />
                </g>

                {/* Pixar-like Cute Round Organelle Citizens */}
                {/* Citizen 1 (Left - confused looking) */}
                <g transform="translate(-110, 30)">
                  <ellipse cx="0" cy="12" rx="14" ry="7" fill="#000000" opacity="0.4" />
                  {/* Round spherical body with soft bioluminescence */}
                  <circle cx="0" cy="0" r="15" fill="#38bdf8" stroke="#bae6fd" strokeWidth="1.5" />
                  {/* Large Expressive Pixar Eyes */}
                  <circle cx="-4" cy="-3" r="4.5" fill="#ffffff" />
                  <circle cx="-3.5" cy="-3" r="2.8" fill="#0f172a" />
                  <circle cx="-4.5" cy="-4" r="1.2" fill="#ffffff" />

                  <circle cx="5" cy="-3" r="4.5" fill="#ffffff" />
                  <circle cx="4.5" cy="-3" r="2.8" fill="#0f172a" />
                  <circle cx="3.5" cy="-4" r="1.2" fill="#ffffff" />
                  {/* Confused tilted antenna */}
                  <path d="M 0 -15 Q -4 -25 -2 -28" fill="none" stroke="#38bdf8" strokeWidth="2" />
                  <circle cx="-2" cy="-28" r="3" fill="#facc15" />
                </g>

                {/* Citizen 2 (Right - worker stopping & turning head) */}
                <g transform="translate(115, 35)">
                  <ellipse cx="0" cy="12" rx="14" ry="7" fill="#000000" opacity="0.4" />
                  <circle cx="0" cy="0" r="15" fill="#c084fc" stroke="#f3e8ff" strokeWidth="1.5" />
                  {/* Large Expressive Pixar Eyes */}
                  <circle cx="-5" cy="-2" r="4.5" fill="#ffffff" />
                  <circle cx="-4" cy="-2" r="2.8" fill="#0f172a" />
                  <circle cx="-5.5" cy="-3" r="1.2" fill="#ffffff" />

                  <circle cx="4" cy="-2" r="4.5" fill="#ffffff" />
                  <circle cx="5" cy="-2" r="2.8" fill="#0f172a" />
                  <circle cx="4" cy="-3" r="1.2" fill="#ffffff" />
                  {/* Antenna */}
                  <path d="M 0 -15 Q 4 -25 2 -28" fill="none" stroke="#c084fc" strokeWidth="2" />
                  <circle cx="2" cy="-28" r="3" fill="#2dd4bf" />
                </g>
              </g>
            </svg>

            {/* Bottom Alert Ribbon */}
            <div className="absolute bottom-4 left-4 right-4 bg-slate-950/80 backdrop-blur-md border border-amber-400/40 rounded-2xl p-3 flex flex-col sm:flex-row items-center justify-between gap-3 z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs sm:text-sm font-extrabold text-amber-200">
                    {scene1State === 'crisis' ? 'Peringatan: Fluktuasi Detak Energi di Pembangkit Utama!' : 'Desa Sel: Operasi Pagi Berjalan Normal'}
                  </div>
                  <div className="text-[11px] text-slate-300">
                    {scene1State === 'crisis' ? 'Tiga fasilitas kehilangan suplai daya, warga kecil menghentikan aktivitas.' : 'Jalur energi berdenyut stabil di seluruh organel desa.'}
                  </div>
                </div>
              </div>
          ...
          <div className="relative w-full aspect-video max-h-[460px] bg-gradient-to-b from-[#022c22] via-[#0f172a] to-[#2e1065] rounded-3xl overflow-hidden border-2 border-teal-500/40 shadow-2xl flex items-center justify-center select-none">
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-slate-950/80 backdrop-blur-md border border-teal-500/40 rounded-2xl p-3 flex flex-col sm:flex-row items-center justify-between gap-3 z-10">
              <div className="text-xs sm:text-sm text-teal-200 font-semibold">
                Panel diagnostik mendeteksi penurunan drastis pada tiga indikator utama Pembangkit Mitokondria.
              </div>
              <button
                type="button"
              ...
              >
                <span>Masuk ke Dashboard Babak 1</span>
      ...
      {/* ========================================================================= */}
      {/* ADEGAN 3: BABAK 1 – OBSERVASI GEJALA (HOLOGRAPHIC DASHBOARD & 3 REAL ASSETS) */}
      {/* ========================================================================= */}
      {currentScene === 'scene3' && (
        <div className="space-y-5">
          {/* Main 3D Photorealistic Holographic Diagnostic Dashboard (BBC / NatGeo Style) */}
          <div className="relative w-full rounded-3xl overflow-hidden border-2 border-teal-500/40 shadow-2xl bg-slate-950 group">
            <div className="relative aspect-video max-h-[380px] w-full overflow-hidden">
              <img
                src={symptomsDashboardImg}
                alt="Panel Dashboard Holografik Krisis Energi Desa Sel"
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setFullscreenImage({
                    src: symptomsDashboardImg,
                    title: 'Adegan 1: Panel Dashboard Holografik Transparan (3D Ilmiah Realistis)',
                    desc: 'Proyeksi hologram transparan melayang di atas lanskap mikroskopis Desa Sel dengan pencahayaan sinematik biru-abu dan aksen sorotan oranye lembut.'
                  });
                }}
          ...
          {/* Three Photorealistic Physical Symptoms with Light-Blue Glowing Ring */}
          <div className="bg-gradient-to-b from-[#042f2e] via-[#0f172a] to-[#1e1b4b] rounded-3xl p-5 sm:p-6 border-2 border-teal-500/40 shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs sm:text-sm font-extrabold text-teal-200 flex items-center gap-2">
                ...
                <span>Pemindaian Tiga Indikator Kerusakan Fisik:</span>
              </span>
              <span className="text-[11px] text-slate-400 hidden sm:inline">
                Klik tiap indikator untuk inspeksi mikroskopis
              </span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Ikon 1: Bohlam Kaca Meredup */}
              <div
                onClick={() => {
                  sfx.playClick();
                  setInspectedSymptom('bulb');
                }}
                className={`relative rounded-2xl p-4 flex flex-col items-center justify-center transition-all cursor-pointer group border-2 ${
                  inspectedSymptom === 'bulb'
                    ? 'bg-amber-500/10 border-amber-400 shadow-lg shadow-amber-500/20'
                    : 'bg-slate-900/60 border-teal-500/30 hover:border-teal-400/60'
                }`}
              >
                {/* Thin Light-blue glowing ring encasing the photorealistic macro asset */}
                <div className="relative w-28 h-28 rounded-full border-2 border-sky-400/90 p-1 flex items-center justify-center shadow-[0_0_20px_rgba(56,189,248,0.5)] mb-3 bg-slate-950 overflow-hidden">
                  <img
                    src={dimBulbImg}
                    alt="Bohlam Kaca Meredup"
                    referrerPolicy="no-referrer"
                    className="w-full h-full object-cover rounded-full transition-transform duration-500 group-hover:scale-110"
                  />
                  <div className="absolute inset-0 rounded-full border border-sky-300/40 pointer-events-none" />
                </div>
                <div className="text-xs font-bold text-amber-200 text-center">Gejala A: Bohlam Kaca Meredup</div>
                <div className="text-[11px] text-slate-300 text-center mt-1 leading-relaxed">
                  Filamen tungsten berpendar redup kekuningan; penerangan biosintesis sel kehilangan intensitas daya.
                </div>
                <button
                  type="button"
                  onClick={(e) => {
                    ...
                    setFullscreenImage({
                      src: dimBulbImg,
                      title: 'Gejala A: Bohlam Kaca dengan Filamen Kuning Meredup',
                      desc: 'Makro realistis bola lampu kaca dengan filamen tungsten yang meredup di bawah atmosfer sinematik biru-abu.'
                    });
                  }}
                  className="mt-2 text-[10px] text-sky-300 hover:text-sky-100 flex items-center gap-1 font-semibold"
                >
                  <Eye className="w-3 h-3" />
                  <span>Inspeksi Makro</span>
                </button>
              </div>

              {/* Ikon 2: Roda Gigi Logam Berkarat Berputar Pelan */}
              <div
                onClick={() => {
                  sfx.playClick();
                  setInspectedSymptom('gear');
                }}
                className={`relative rounded-2xl p-4 flex flex-col items-center justify-center transition-all cursor-pointer group border-2 ${
                  inspectedSymptom === 'gear'
                    ? 'bg-amber-500/10 border-amber-400 shadow-lg shadow-amber-500/20'
                    : 'bg-slate-900/60 border-teal-500/30 hover:border-teal-400/60'
                }`}
              >
                ...
                <div className="relative w-28 h-28 rounded-full border-2 border-sky-400/90 p-1 flex items-center justify-center shadow-[0_0_20px_rgba(56,189,248,0.5)] mb-3 bg-slate-950 overflow-hidden">
                  <img
                    src={rustyGearsImg}
                    alt="Roda Gigi Logam Berkarat"
                    referrerPolicy="no-referrer"
                    className="w-full h-full object-cover rounded-full transition-transform duration-500 group-hover:scale-110"
                  />
                  <div className="absolute inset-0 rounded-full border border-sky-300/40 pointer-events-none" />
                </div>
                <div className="text-xs font-bold text-amber-200 text-center">Gejala B: Roda Gigi Logam Berkarat</div>
                <div className="text-[11px] text-slate-300 text-center mt-1 leading-relaxed">
                  Permukaan metalik berpatina aus dan berputar pelan; protein motor molekuler kehilangan tenaga dorong.
                </div>
                <button
                  type="button"
                  onClick={(e) => {
                    ...
                    setFullscreenImage({
                      src: rustyGearsImg,
                      title: 'Gejala B: Roda Gigi Logam Berkarat Berputar Pelan',
                      desc: 'Fotografi makro detail roda gigi mekanik dengan korosi besi oranye-kecoklatan dan tekstur metalik natural.'
                    });
                  }}
                  className="mt-2 text-[10px] text-sky-300 hover:text-sky-100 flex items-center gap-1 font-semibold"
                >
              ...
              {/* Ikon 3: Gerbang Kayu-Logam Berkarat Oranye-Kecoklatan */}
              <div
                onClick={() => {
                  sfx.playClick();
                  setInspectedSymptom('gate');
                }}
                className={`relative rounded-2xl p-4 flex flex-col items-center justify-center transition-all cursor-pointer group border-2 ${
                  inspectedSymptom === 'gate'
                    ? 'bg-amber-500/10 border-amber-400 shadow-lg shadow-amber-500/20'
                    : 'bg-slate-900/60 border-teal-500/30 hover:border-teal-400/60'
                }`}
              >
                ...
                <div className="relative w-28 h-28 rounded-full border-2 border-sky-400/90 p-1 flex items-center justify-center shadow-[0_0_20px_rgba(56,189,248,0.5)] mb-3 bg-slate-950 overflow-hidden">
                  <img
                    src={rustyGateImg}
                    alt="Gerbang Kayu-Logam Berkarat"
                    referrerPolicy="no-referrer"
                    className="w-full h-full object-cover rounded-full transition-transform duration-500 group-hover:scale-110"
                  />
                  <div className="absolute inset-0 rounded-full border border-sky-300/40 pointer-events-none" />
                </div>
                <div className="text-xs font-bold text-amber-200 text-center">Gejala C: Gerbang Kayu-Logam Aus</div>
                <div className="text-[11px] text-slate-300 text-center mt-1 leading-relaxed">
                  Tekstur kayu dan pelat besi teroksidasi karat oranye; pompa ion membran melemah menjaga gradien sel.
                </div>
                <button
                  type="button"
                  onClick={(e) => {
                    ...
                    setFullscreenImage({
                      src: rustyGateImg,
                      title: 'Gejala C: Gerbang Kayu-Logam dengan Tekstur Karat Detail',
                      desc: 'Makro tekstur kayu organik lapuk dan pelat besi berkarat oranye-kecoklatan yang mencerminkan hilangnya integritas transpor membran.'
                    });
                  }}
                  className="mt-2 text-[10px] text-sky-300 hover:text-sky-100 flex items-center gap-1 font-semibold"
                >
        ...
        <div className="space-y-5">
          ...
          <div className="bg-gradient-to-b from-[#042f2e] via-[#0f172a] to-[#2e1065] rounded-3xl p-4 sm:p-6 border-2 border-teal-500/40 shadow-xl space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex rounded-xl bg-slate-900/90 p-1 border border-teal-500/30 gap-1 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => {
                    sfx.playClick();
            ...
            {/* Display Area: Either 3D Photorealistic or Schematic */}
            {viewModeBabak2 === 'photo' ? (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2">
            ...
            {isPassed && (
              ...
              >
                ...
                <span>Selesaikan Level 4 &amp; Buka Level 5</span>
              </button>
            )}
          </div>
        </div>
