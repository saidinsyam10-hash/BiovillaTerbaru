...
import { getAssetCandidates, getMediaObjectURL, onMediaUpdated } from '../utils/mediaStore';
...
export const HotspotIllustration: React.FC<HotspotIllustrationProps> = ({
  ...
  const loadMedia = useCallback(async () => {
    ...
    // 1. Check dedicated hotspot key in IndexedDB
    const hotspotKey = `hotspot_${hotspotId}`;
    const byHotspot = await getMediaObjectURL(hotspotKey);
    if (byHotspot) {
      setCustomSrc(byHotspot);
      return;
    }

    // 2. Check cleanKey in IndexedDB
    if (cleanKey) {
      const byCleanKey = await getMediaObjectURL(cleanKey);
      if (byCleanKey) {
